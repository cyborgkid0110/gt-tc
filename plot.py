...
def tx_power_plot(network, node_dict):
    # Create directed graph
    G = nx.DiGraph()
    positions = {}  # Map: index -> (x, y)
    ...
    # Get p_ch values for coloring
    p_ch_values = [node_dict[pos]['power'] for pos in network['vertices']]

    # Draw the graph
    plt.figure(figsize=(10, 8))
    nodes = nx.draw_networkx_nodes(G, positions, node_color=p_ch_values, cmap=plt.cm.RdYlBu, vmin=P_MIN, vmax=P_MAX, node_size=100)
    nx.draw_networkx_edges(G, positions, edge_color='gray', arrows=True, arrowstyle='->', alpha=0.3)
    nx.draw_networkx_labels(G, positions, labels={i: str(i) for i in G.nodes()}, font_size=5, font_color='black')
